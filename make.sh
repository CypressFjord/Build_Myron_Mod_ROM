...
rm -rf "$GITHUB_WORKSPACE"/images/product/pangu/system/app/NQNfcNci
...
Start_Time
sudo $a7z a "$GITHUB_WORKSPACE"/zip/Myron-Mod-HyperOS-${model}-${port_os_version}-CypressFjord.zip "$GITHUB_WORKSPACE"/firmware/* >/dev/null
sudo rm -rf "$GITHUB_WORKSPACE"/images
End_Time 生成ZIP刷机包
#定制ROM包名
echo -e "${Red}- 开始定制ROM包名${NC}"
Start_Time
md5=$(md5sum "$GITHUB_WORKSPACE"/zip/Myron-Mod-HyperOS-${model}-${port_os_version}-CypressFjord.zip)
echo "MD5=${md5:0:32}" >>$GITHUB_ENV
zip_md5=${md5:0:10}
rom_name="Myron-Mod-HyperOS-${model}-${port_os_version}-CypressFjord-${zip_md5}.zip"
sudo mv "$GITHUB_WORKSPACE"/zip/Myron-Mod-HyperOS-${model}-${port_os_version}-CypressFjord.zip "$GITHUB_WORKSPACE"/zip/"${rom_name}"
